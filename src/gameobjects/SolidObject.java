...
/**
 * Parent class to all 'solid' gameobjects where 'solid' means that the object has a body that is a part of the
 * gameworld (the world of the physics library). This class contains shared drawing functions and fields.
 */
public class SolidObject {
    ...
    protected final long ID;   //The id of the solid object
    ...
    /**
     * Sets variables for a Solid Object with static position and collision properties. Note that the object will not
     * be drawn unless a image or color is added first.
     * @param pos The position at which to place the Object (units in meters)
     ...
     * @param gc The GraphicsContext to be used to draw with
     */
    protected void drawCircle(GraphicsContext gc, Vec2 position, Double radius){
        //Saving the current xy-plane to the gc stack
        gc.save();
        //Translating the original gc xy-plane to a new xy-plane with its origin in the center of this body and saving the
        //new xy-plane on top of the stack
        ...
        //Drawing the body so that the center of the visual representation is in the new xy-planes origin
        gc.setFill(color);
        float pixRadius = GameComponent.metersToPix(radius.floatValue());
        if (image == null) {gc.fillOval(-pixRadius, -pixRadius, 2 * pixRadius, 2 * pixRadius);}
        else {gc.drawImage(image, -pixRadius, -pixRadius);}
        //Popping the stack, removing the top element, thus leaving the original xy-plane at the top
        gc.restore();
    }
     ...
     * @param gc The 'GraphicsContext' with which to paint.
    ...
    protected void drawCircleFixture(GraphicsContext gc, Fixture fixture){
        //Casting the attached 'Shape' to CircleShape (this must be a CircleShape for the Fixture to be a circle)
        ...
        //Drawing the circle
        drawCircle(gc, fixturePos, radius.doubleValue());
    }
     ...
     * used in collision checks and in the 'gamelogic.Map' where objects are staged for removal/addition to check so that objects are exactly
